...
namespace _00.Work.WorkSpace.CheolYee._04.Scripts.Core.Items.UI
{
    ...
    {
        [Header("Refs")]
        public GridInventoryManager grid; //그리드 로직/UI 좌표 변환
        ...
        private void Start()
        {
            ...
            if (gridSlots != null) gridSlots.BuildSlots(); //슬롯 생성
            if (gridSlots != null) gridSlots.RefreshColors(); //색 초기화
        }
        ...
        private void Update()
        {
            ...
            if (hover != null)
            {
                ...
                else
                {
                    // 드래그 중이 아닐 때만 호버 표시
                    bool oob;
                    var cell = grid.ScreenToCell(uiCamera, Input.mousePosition, out oob);
        ...
        private void HandleInput()
        {
            ...
            {
                bool oob;
                Vector2Int cell = grid.ScreenToCell(uiCamera, Input.mousePosition, out oob);
                if (!oob)
                {
                    var picked = grid.GetItemAtCell(cell);
                    if (picked != null)
                    {
                        // 시각화 제거
                        if (placedLayer != null) placedLayer.HideItem(picked);

                        var so = _dataCache[picked.dataId];
                        grid.Remove(picked);
            ...
            Vector2Int cell2 = grid.ScreenToCell(uiCamera, Input.mousePosition, out var oob2); //셀 좌표 추출

            if (_dragItem != null)
            {
                UpdateGhost(cell2, !oob2);
            }
        ...
        private void UpdateGhost(Vector2Int anchorCell, bool inside)
        {
            ...
            bool ok = inside && grid.CanPlace(_dragItem, _dragData, anchorCell, _dragRotation); //가능 여부 검사
            _ghost.SetOk(ok);
            
            //유령 타일들의 고정 위치 목록 만들기
            List<Vector2Int> absCells = grid.GetAbsoluteCells(_dragItem, _dragData, anchorCell, _dragRotation);
            var anchors = new List<Vector2>(absCells.Count);
        ...
        private void TryPlace(Vector2Int anchorCell, bool inside)
        {
            ...
            if (grid.CanPlace(_dragItem, _dragData, anchorCell, _dragRotation))
            {
                ...
                {
                    List<Vector2Int> abs = grid.GetAbsoluteCells(_dragItem, _dragData, anchorCell, _dragRotation);
                    placedLayer.ShowItem(_dragItem, _dragData, abs);
                }
